...
public class BarcodeScanActivity extends Activity implements ZXingScannerView.ResultHandler {
    ...
    public void handleResult(Result rawResult) {
        ...
        intent.putExtra("barcode_format", rawResult.getBarcodeFormat().toString());
        startActivity(intent);

        // Do something with the result here
        Log.v(TAG, "Scan result: " + rawResult.getText()); // Prints scan results
        Log.v(TAG, "Scan Format: " + rawResult.getBarcodeFormat().toString()); // Prints the scan format (qrcode, pdf417 etc.)
